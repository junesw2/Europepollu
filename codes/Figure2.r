...
### Read pollution-weighted data

# Define the folder path
foldout <- paste0("/Yourfolder/inputdata/")
# Read the RDS file containing pollution data
SFO_NUTS_CASE <- readRDS(file = paste0(foldout, "poldata_gpw_pop.Rdata"))

# Generate maps for PM2.5
varplot <- "pm25v0_pw"
palette <- c4a('brewer.or_rd')
maps_pm25 <- map_generator(countries = SFO_COU_WORLD,
                           nuts0 = SFO_COU_EU,
                           palette = palette,
                           style = "pretty",
                           title1 = "a) Average PM2.5",
                           nuts.var = SFO_NUTS_CASE,
                           varplot = varplot)
...
maps_pm10 <- map_generator(countries = SFO_COU_WORLD,
                           ...
                           palette = palette,
                           style = "pretty",
                           title1 = "c) Average PM10",
                           nuts.var = SFO_NUTS_CASE,
                           varplot = varplot)

# Generate maps for O3
varplot <- "o3_8hpred_pw"
palette <- c4a('brewer.or_rd')
maps_o3 <- map_generator(countries = SFO_COU_WORLD,
                         nuts0 = SFO_COU_EU,
                         palette = palette,
                         breaks = c(65, 70, 75, 80, 85, 90, 95),
                         title1 = "g) Average O3",
                         nuts.var = SFO_NUTS_CASE,
                         varplot = varplot)
...
maps_no2 <- map_generator(countries = SFO_COU_WORLD,
                          ...
                          palette = palette,
                          style = "pretty",
                          title1 = "e) Average NO2",
                          nuts.var = SFO_NUTS_CASE,
                          varplot = varplot)

### Read pollution-weighted trend data

# Define the folder path
foldout <- paste0("/Yourfolder/docs/scriptforNC/")
# Read the RDS file containing pollution trend data
SFO_NUTS_CASE <- readRDS(file = paste0(foldout, "poldata_gpw_pop_trend.Rdata"))

# Generate maps for PM2.5 trends
varplot <- "pm25v0_pw"
maps_pm25_slope <- map_generator(countries = SFO_COU_WORLD,
                                 nuts0 = SFO_COU_EU,
                                 c4col = "kovesi.div_bu_wh_rd2",
                                 mid = 0,
                                 breaks = c(-5, -4, -3, -2, -1, 0, 1, 2),
                                 title1 = "b) Trend in PM2.5",
                                 nuts.var = SFO_NUTS_CASE,
                                 varplot = varplot)

# Generate maps for PM10 trends
varplot <- "pm10v0_pw"
maps_pm10_slope <- map_generator(countries = SFO_COU_WORLD,
                                 nuts0 = SFO_COU_EU,
                                 c4col = "kovesi.div_bu_wh_rd2",
                                 mid = 0,
                                 style = "pretty",
                                 breaks = c(-5, -4, -3, -2, -1, 0, 1, 2),
                                 title1 = "d) Trend in PM10",
                                 nuts.var = SFO_NUTS_CASE,
                                 varplot = varplot)

# Generate maps for O3 trends
varplot <- "o3_8hpred_pw"
maps_o3_slope <- map_generator(countries = SFO_COU_WORLD,
                               nuts0 = SFO_COU_EU,
                               c4col = "kovesi.div_bu_wh_rd2",
                               mid = 0,
                               style = "pretty",
                               breaks = c(-0.5, -0.25, -0.125, 0, 0.125, 0.25, 0.5, 1),
                               title1 = "h) Trend in Ozone",
                               nuts.var = SFO_NUTS_CASE,
                               varplot = varplot)

# Generate maps for NO2 trends
varplot <- "no2pred_pw"
maps_no2_slope <- map_generator(countries = SFO_COU_WORLD,
                                nuts0 = SFO_COU_EU,
                                c4col = "kovesi.div_bu_wh_rd2",
                                mid = 0,
                                style = "pretty",
                                breaks = c(-5, -4, -3, -2, -1, 0, 1, 2),
                                title1 = "f) Trend in NO2",
                                nuts.var = SFO_NUTS_CASE,
                                varplot = varplot)

# Create the plots and save as PDF
library(grid)
foldout <- paste0("/Yourfolder/docs/scriptforplot/figure/")
if (!file_test("-d", foldout)) {
  dir.create(foldout, recursive = TRUE)
}
pdf(paste0(foldout, "Figure2_prop_gpw.pdf"), width = 10, height = 20)
# Create a 2X2 grid
grid.newpage()
...
plot11 <- maps_pm25_slope$europe
plot21 <- maps_pm10_slope$europe
plot31 <- maps_no2_slope$europe
plot41 <- maps_o3_slope$europe

# Add the plots to the grid
print(plot1, vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(plot11, vp = viewport(layout.pos.row = 1, layout.pos.col = 2))
print(plot2, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(plot21, vp = viewport(layout.pos.row = 2, layout.pos.col = 2))
print(plot3, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
print(plot31, vp = viewport(layout.pos.row = 3, layout.pos.col = 2))
print(plot4, vp = viewport(layout.pos.row = 4, layout.pos.col = 1))
print(plot41, vp = viewport(layout.pos.row = 4, layout.pos.col = 2))
dev.off()
